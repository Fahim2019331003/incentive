...
import getUserTableData from '@/app/actions/getUserTableData';
...
const UserTableAll = ({userEmail,tableType}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response: any = await getUserTableData(userEmail,tableType);
        // const response=[];
  ...
  const filterItems = useMemo(async () => {
    if (hasSearchFilter) {
      // const temp_data: any = await getUserSearchData(search, tableType);
      const temp_data=[];
      setDatas(temp_data);
    } else {
      setDatas(tableData);
      ...
      <div className="relative overflow-x-auto mb-10 mt-3">
        <div className="mt-5 text-3xs">
          ...
          {loading ? (
            ...
            >
              <TableHeader columns={columns[tableType]}>
                {(column:any) => (
