...
import getTableAssignedData from '@/app/actions/getTableAssignedData';
...
import { SearchIcon } from '../applications/SearchIcon';
...
const TablePendingEvaluate = ({user}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response: any = await getTableAssignedData(user.id);
  ...
  const onCloseHandlerA = async () => {
    ...
    if (
      ...
      const newData: any = await getTableAssignedData(user.id);
      setDatas(newData);
            ...
            <ModalContent>
              {(onCloseA) => (
                <>
                  ...
                  <ModalBody>
                    <p>
                      Do you want to ACCEPT {Array.from(selectedKeys).length}{' '}
            ...
            <ModalContent>
              {(onCloseB) => (
                <>
                  ...
                  <ModalBody>
                    <p>
                      Do you want to REJECT {Array.from(selectedKeys).length}{' '}
                      application(s)?
                    </p>
                    <div>
                      <Textarea
...
export default TablePendingEvaluate;
